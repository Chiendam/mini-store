                            ...
                            <div class="panel-body row">
                                ...
                                <div class="form-group col-md-6">
                                    <label class="control-label text-bold">Nhóm quyền<span
                                            class="text-danger">*</span></label>
                                    <div>
                                        <select id="selectIsActive" name="role"
                                                class="bootstrap-select form-control select-lg">
                                            <option selected
                                                    disabled>Chọn quyền</option>
                                            @forelse(@$roles ?? [] as $role)
                                                <option {{old('role', @$user->role) == $role['value'] ? 'selected' : ''}} value="{{$role['value']}}">{{$role['name']}}</option>
                                            @empty
                                            @endforelse
                                        </select>
                                        @error('role')
                        ...
                        <div class="panel panel-white">
                            ...
                            <div class="panel-body">
                                <div>
                                    ...
                                    <a href="{{ route('admin.dashboard') }}" class="btn btn-default"><i
                                            class=" icon-close2"></i>
                                        Đóng</a>
                                </div>
